...
// ⚠️ amount(금액)는 공고 원문 대조 전 초기값이다. keys에 "// 별칭" 표시된 것은 현장에서 자주 쓰는
//    표현을 한국표준산업분류상 같은 분류의 기존 티어에 연결한 것 — 금액을 바꾼 게 아니다.
// ⚠️ "서비스업"처럼 세부 업종을 알 수 없는 포괄어는 일부러 넣지 않는다(미매칭 → 배지 공란).
...
// 대소문자 무시 (pc방 / PC방 둘 다 매칭 — 한글은 toUpperCase 영향 없음)
function bizScaleCap(industry) {
  ...
  for (var i = 0; i < BIZ_SCALE_TABLE.length; i++) {
    ...
    for (var j = 0; j < row.keys.length; j++) {
      if (su.indexOf(row.keys[j].toUpperCase()) >= 0) return { amount: row.amount, group: row.group, key: row.keys[j] };
    }
  }
  return null;
}
