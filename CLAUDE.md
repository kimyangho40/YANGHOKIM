...
## 🔴 다음 세션 우선 처리 — `work_notes` 개인노트 비공개가 DB에서 안 막혀 있다
(2026-08-01 조사. "소통내역 → 업무노트 연결" 작업 중 발견. **이번 작업 범위에서는 의도적으로 제외**했다.)

업무노트는 화면상 "본인 것만 보임"인데, **실제 차단은 클라이언트 필터뿐**이다.

- 정책(카탈로그 실측): `p_work_notes_all` = `{authenticated}` / `ALL` / `using(is_approved())`
  → **사용자별 스코프가 없다.** 승인된 로그인 사용자면 누구나 전건 조회 가능.
- `fetchNotes`(App.js)는 `select("*")`로 **전건을 받아온 뒤** `wnViewable(myName)`으로 화면에서만 거른다.
  (`wnViewable`: 양호=전원 / 미현·인선=서로 / 나머지=본인만)
- 즉 브라우저 네트워크 탭이나 로그인 토큰으로 직접 조회하면 **남의 노트가 그대로 보인다.**
  anon key는 `revoke` 돼 있어 비로그인 유출은 아니다 — **내부 사용자 간** 문제다.
- `team_notes`는 원래 공유용이라 해당 없음.

고칠 때 주의: 정책을 "추가"하면 OR로 합쳐져 더 느슨해진다(아래 2-2 참고).
반드시 기존 `p_work_notes_all`을 **drop 하고** 새로 만들 것. 그리고 `assignee`가
`profiles.name` 텍스트라 `auth.uid()`와 직접 비교가 안 된다 — `profiles`를 조인하는
서브쿼리나 `created_by` 기준 정책이 필요하고, **공유그룹(미현·인선)·관리자(양호) 예외도 정책에 반영**해야
화면 동작이 안 깨진다. 조인 정책은 성능도 확인할 것.
