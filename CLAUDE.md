...
- ⚠️ **`fetchData` 는 `agency_cases` 를 승인 이상 상태(`승인·약정·완료·자금집행완료`)만 불러온다.**
  그래서 **승인 전 건만 있는 해는 `yearRange` 에 안 잡힌다** — 2026-08-19 기준 2025년이 그렇다
  (`agency_cases` 에 2025년 20건이 있지만 전부 승인 이전이라 화면에 안 온다).
  그 해도 열려면 `fetchData` 의 status 필터를 손봐야 한다. **연도 UI 만으로는 안 열린다.**
