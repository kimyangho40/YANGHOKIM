...
- 확인창은 **삭제에만** 있다(요청 사항). 완료는 카드에서 ↩️ 로 되돌릴 수 있어 확인창을 두지 않았다.
  단 **체크리스트 없는 카드의 완료(`markDone`)는 되돌리는 UI 가 없다** — '완료된 업무 N건 보기' 로 보이기만 한다.
- 남은 사소한 점: 카드의 진행률 바는 `takenCount`(가져간 수)만 센다 → ✅ 로 끝낸 항목은 그 숫자에 안 들어간다.
  라벨이 "N/M 가져감" 이라 틀린 말은 아니지만, 나중에 "완료 포함"으로 바꾸려면 라벨도 같이 고칠 것.
