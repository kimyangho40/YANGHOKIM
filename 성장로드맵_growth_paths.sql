-- 성장 로드맵 · growth_paths + growth_roadmap_copy (읽기 전용 참조 테이블)
-- 짝: src/pages/GrowthRoadmap.jsx · 원본 패키지: growth-roadmap-package/supabase_growth_roadmap.sql
-- 되돌리기: 성장로드맵_growth_paths_rollback.sql / 검증: 성장로드맵_growth_paths_검증.sql
-- 재실행 안전(upsert). 기존 테이블 변경 0건 · 새 컬럼 0개.
--
-- ⚠️ 첨부 원본을 그대로 쓰지 않고 보안 부분만 다시 썼다. 「기관 사이트」(agency_sites) 때와
--    똑같은 문제라 그때 만든 `기관사이트_agency_sites_보안보정.sql` 을 선례로 삼는다.
--    ① 원본 정책이 `to authenticated using (true)` 라 미승인(pending)·퇴사(rejected) 계정도
--       세션만 살아 있으면 읽힌다. 이 저장소는 `profiles.status='rejected'` 를 즉시 전면
--       차단 수단으로 쓰므로(CLAUDE.md 2-3·2-4) 이 테이블만 예외가 되면 안 된다.
--    ② public 스키마의 postgres 기본권한이 `anon=arwdm` 이라 **새 테이블에 anon 의
--       INSERT/SELECT/UPDATE/DELETE 가 자동으로 붙는다.** RLS 가 켜져 있어 지금 새지는
--       않지만, 2026-07-27 사고가 정확히 "정책 하나 느슨해지면 남은 GRANT 로 뚫린다"였다.
--    ③ TRUNCATE 는 RLS 도 행 트리거도 우회한다 → authenticated 에서 회수(CLAUDE.md 2-2).
--
-- 시드 16건(경로) + 문구 1행은 첨부 원본에서 한 글자도 바꾸지 않고 옮겼다.
-- `source` 컬럼: video 12건(영상 판독) / authored 4건(같은 문체로 채운 것 — 원문 대조 필요).
...
--    반드시 같은 이름으로 drop 후 재생성한다. 원본 이름도 같이 지운다(원본을 먼저 돌렸을 경우).
drop policy if exists "growth_paths read for authenticated" on public.growth_paths;
create policy "growth_paths read for authenticated"
  on public.growth_paths for select to authenticated
  using (public.is_approved());

drop policy if exists "growth_copy read for authenticated" on public.growth_roadmap_copy;
create policy "growth_copy read for authenticated"
  on public.growth_roadmap_copy for select to authenticated
  using (public.is_approved());
...
-- anon 은 한 톨도 주지 않는다
revoke all on public.growth_paths        from anon;
revoke all on public.growth_roadmap_copy from anon;

-- 앱(PostgREST)이 쓰지 않는 위험 권한 회수
revoke truncate, references, trigger on public.growth_paths        from authenticated, anon;
revoke truncate, references, trigger on public.growth_roadmap_copy from authenticated, anon;

-- ⚠️ 이 둘은 **읽기 전용 참조 테이블**이라 쓰기 권한도 회수한다.
--    public 스키마 기본권한이 `authenticated=arwdm` 이라 새 테이블에 INSERT/UPDATE/DELETE 가
--    자동으로 붙는다(실측으로 확인 — 위 truncate 회수만으로는 6건이 남아 있었다).
--    지금은 쓰기 정책이 없어 RLS 가 막지만, 정책 하나가 느슨해지면 남은 GRANT 로 뚫린다
--    (2026-07-27 사고 패턴). 경로 수정은 Supabase 대시보드(service_role)에서 하므로
--    앱 사용자에게 쓰기 권한이 필요 없다.
revoke insert, update, delete on public.growth_paths        from authenticated, anon;
revoke insert, update, delete on public.growth_roadmap_copy from authenticated, anon;
